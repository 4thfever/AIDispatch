...
class PowerFlow():
    ...
    def read_data(self):
        ...
        """
        df_grid = pd.read_csv('D:\\OneDrive\\program\\AIDispatch\\PowerFlow\\input.csv')
        self.num_node = int(df_grid.loc[:,['node_i','node_j']].max().max())
        self.list_type = ['line', 'tran', 'gene', 'load']
        list_num = np.zeros([4,1])
        for i in range(4):
            list_num[i] = int(df_grid[df_grid['type']==self.list_type[i]].shape[0])
