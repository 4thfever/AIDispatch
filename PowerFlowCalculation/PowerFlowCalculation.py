...
def read_data():
    ...
    gv.num_node = int(df_grid.loc[:,['node_i','node_j']].max().max())
    list_attr = [gv.num_line, gv.num_tran, gv.num_gene, gv.num_load]
    list_device = ['line', 'tran', 'gene', 'load']
    list_handle_device = [gv.Line, gv.Tran, gv.Gene, gv.Load]
    list_handles_device = [gv.line, gv.tran, gv.gene, gv.load]
    for i in range(4):
        list_attr[i] = int(df_grid[df_grid['type']==list_device[i]].shape[0])
    gv.num_line, gv.num_tran, gv.num_gene, gv.num_load = list_attr
    gv.error_max = 1e-5
    print(list_attr, gv.error_max, gv.num_line, gv.num_tran, gv.num_gene, gv.num_load) 
    ...
    for type_num in range(4):
        ...
        print(list_handles_device[type_num])
        for i in range(values.shape[0]):
            if 'node_j' in df_buf.columns:
                input_ = [int(ele) if i <= 1 else ele for i, ele in enumerate(values[i])]
            else:
                input_ = [int(ele) if i == 0 else ele for i, ele in enumerate(values[i])]
